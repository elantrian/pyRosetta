...
print initial_pose.sequence()

#mutate residue and minimize
for entry in sorted(mutationDict):
	seq_pos = int(entry[:-1])
	old_res = initial_pose.residue(seq_pos).name()
	...
	if args.minimize:
		...
		while iterator < 51:
			...
			mutant_pose.dump_pdb(str("SUMO_"+variant_name+str(iterator)+"_min.pdb"))
			variant_score = sf(mutant_pose)
			iterator+=1

			data.append(variant_name+str(iterator) + str(old_res) + "," + \
	                str(variant_score) + "," + \
